...
class PerforatorScheduler(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        if (self.verbose):
            if (abs(self.lr_at_epoch_start - self.target_lr) >= sys.float_info.min):
                print("LR will change from "+str(self.lr_at_epoch_start)+" to "+str(self.target_lr))
            else:
                print("LR will be stable at "+str(self.lr_at_epoch_start))
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current is None:
            ...
            if self.monitor_op(current, self.best):
                ...
                self.wait += 1
                print(str(self.wait)+ " / " + str(self.patience))
